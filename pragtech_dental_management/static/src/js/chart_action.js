...
//
												...
												if (found_selected_tooth[0]) {
													if (!found_selected_tooth[0])
														alert('Please select the surface first ##check only tooth####!');
													else {
														var found = self.$el.find('.selected_treatment');
														if (found) {
															found.removeClass("selected_treatment");
												...
												if (!found_selected_tooth[0] && !selected_surface[0]) {
													alert('Please select the surface first ###check both tooth and surface###!');
													return;
												}
		...
		write_patient_history : function(self_var, res) {
			...
			_.each(res, function(each_operation) {
				...
				if (each_operation['tooth_id']){
					...
					var ts = imageMapList ;
                    var tooth_surface = ts.slice(0,-3);
//                    var ts = each_operation['tooth_id']  ;
//                    var tooth_surface = imageMapList;
                  ...
                  //   keyList.push(imageMapList+'_'+ts);
					...
					/*CHANGED HERE*/
					self_var.put_data(self_var, each_operation['surface'].split(' '),
...
//				status_to_define_temp = status_to_define.substr(0, 1).toUpperCase() + status_to_define.substr(1);
				...
				this.get_treatment_charge(selected_treatment_temp.treatment_id).then(function(t_charge) {
					...
					console.log("selected_treatment_temp", selected_treatment_temp)
...
//								}
		...
		/*CHANGED HERE*/
		put_data : function(self_var, surfaces,
							selected_tooth_temp,
							selected_surface_temp,
							status_defined,
							created_date,
			...
			this.get_treatment_charge(selected_treatment_temp.treatment_id).then(function(t_charge) {
				...
				if (status_defined != 'completed') {
					if (dignosis) {
						if (dignosis.id) {
							var html;
							for (var i = 0; i < dignosis_records[0].length; i++) {
								if (i == (dignosis.id - 1)) {
									html += '<option value = "' + dignosis_records[0][i].id + '" selected="selected">' +
										dignosis_records[0][i].code + '</option>';
								}
								else {
									html += '<option value = "' + dignosis_records[0][i].id + '" >' +
										dignosis_records[0][i].code + '</option>';
								}
							}
							table_str += '<td id = "dignosis_' + operation_id +
								'"><select id = "dignosis_code_' +
								operation_id + '">' + html + '</select></td>';
						}
						else {
							var html;
							for (var i = 0; i < dignosis_records[0].length; i++) {
               ...
               if(!tooth_by_part){
				...
				$('#dignosis_code_' + operation_id).change(function () {
					...
					rpc.query({
		                model: 'diagnosis',
		                method: 'get_dignosis_description',
		                args: [parseInt($('#dignosis_code_' + ops_id).val())],
		            }).then(function (res) {
